...
$t->is($v->clean('18 october 1995'), '1995-10-18', '->clean() accepts dates parsable by strtotime');
...
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15)), '1995-10-15', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => '1995', 'month' => '10', 'day' => '15')), '1995-10-15', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => '', 'month' => '', 'day' => '')), null, '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 2020, 'month' => 02, 'day' => 29)), '2020-02-29', '->clean() recognises a leapyear');
...
try
{
  $v->clean(array('year' => 2020, 'month' => 2, 'day' => 30));
  $t->fail('->clean() throws a sfValidatorError if the date is not valid');
  $t->skip('', 1);
}
catch (sfValidatorError $e)
...
$t->is($v->clean('18/10/1995'), '1995-10-18', '->clean() accepts a regular expression to match dates');
$t->is($v->clean(array('year' => '1995', 'month' => '10', 'day' => '18')), '1995-10-18', '->clean() accepts a regular expression when cleaning an array');
...
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 12, 'minute' => 10, 'second' => 15)), '1995-10-15 12:10:15', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => '1995', 'month' => '10', 'day' => '15', 'hour' => '12', 'minute' => '10', 'second' => '15')), '1995-10-15 12:10:15', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => '', 'month' => '', 'day' => '', 'hour' => '', 'minute' => '', 'second' => '')), null, '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 12, 'minute' => 10, 'second' => '')), '1995-10-15 12:10:00', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 12, 'minute' => 10)), '1995-10-15 12:10:00', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 0, 'minute' => 10)), '1995-10-15 00:10:00', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => '0', 'minute' => 10)), '1995-10-15 00:10:00', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 10)), '1995-10-15 10:00:00', '->clean() accepts an array as an input');
$t->is($v->clean(array('year' => 1995, 'month' => 10, 'day' => 15, 'hour' => 0)), '1995-10-15 00:00:00', '->clean() accepts an array as an input');
try
{
  $v->clean(array('year' => 1995, 'month' => 1, 'day' => 15, 'hour' => 12, 'minute' => '', 'second' => 12));
...
$t->is($v->clean('18 october 1995 12:30'), '1995-10-18 12:30:00', '->clean() can accept date time with the with_time option');
$t->is($v->clean(time()), date('Y-m-d H:i:s', time()), '->clean() can accept date time with the with_time option');
$v->setOption('date_format', '~(?P<day>\d{2})/(?P<month>\d{2})/(?P<year>\d{4})~');
$t->is($v->clean('18/10/1995'), '1995-10-18 00:00:00', '->clean() can accept date time with the with_time option');
$v->setOption('date_format', '~(?P<day>\d{2})/(?P<month>\d{2})/(?P<year>\d{4}) (?P<hour>\d{2})\:(?P<minute>\d{2})~');
$t->is($v->clean('18/10/1995 12:30'), '1995-10-18 12:30:00', '->clean() can accept date time with the with_time option');
$v->setOption('date_format', null);
